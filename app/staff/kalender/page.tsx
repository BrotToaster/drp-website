...
export default async function StaffCalendarPage() {
  ...
  const fields = (event?: typeof events[number]) => {
    ...
    return <>
      ...
      <div className="grid gap-4 md:grid-cols-2"><label className="field-label">Beginn<input className="field" type="datetime-local" name="startsAt" defaultValue={revision ? dateTimeLocalValue(revision.startsAt) : ""} required /></label><label className="field-label">Ende<input className="field" type="datetime-local" name="endsAt" defaultValue={revision ? dateTimeLocalValue(revision.endsAt) : ""} required /></label><label className="field-label">Ort<input className="field" name="location" defaultValue={revision?.location || ""} /></label><label className="field-label">Externer HTTPS-Link<input className="field" type="url" name="externalUrl" defaultValue={revision?.externalUrl || ""} /></label></div>
