...
export default async function WebsiteAdminPage() {
  ...
  const links = linkSetting?.value && typeof linkSetting.value === "object" && !Array.isArray(linkSetting.value) ? linkSetting.value as { discordUrl?: string; robloxUrl?: string; discordSupportUrl?: string; melonlyUrl?: string } : {};
  ...
  return (
    <PortalShell authorization={authorization} title="Startseite & Rollenkarten" description="Rollenbereiche, Bilder, Verlinkungen und öffentliche Einstiegslinks verwalten." section="admin">
      ...
      <section className="surface p-6"><p className="eyebrow">Verknüpfungen</p><h2 className="mt-2 text-xl font-semibold">Öffentliche Links & Übergaben</h2><p className="mt-2 text-sm text-[#858b90]">Community-Support bleibt auf Discord; Aktivität, Bewerbungen und Moderationsmaßnahmen bleiben in Melonly.</p><ReliableActionForm action={savePublicLinksAction} className="mt-5 grid gap-4 md:grid-cols-2"><label className="field-label">Discord-Einladung<input className="field" name="discordUrl" type="url" defaultValue={links.discordUrl || siteConfig.discordUrl} required /></label><label className="field-label">Discord-Support / Tickets<input className="field" name="discordSupportUrl" type="url" defaultValue={links.discordSupportUrl || links.discordUrl || siteConfig.discordUrl} required /></label><label className="field-label">Roblox-Beitrittslink<input className="field" name="robloxUrl" type="url" defaultValue={links.robloxUrl || siteConfig.robloxUrl} required /></label><label className="field-label">Melonly-Portal<input className="field" name="melonlyUrl" type="url" defaultValue={links.melonlyUrl || "https://melonly.xyz/dashboard"} required /></label><div className="md:col-span-2"><SubmitButton>Links speichern</SubmitButton></div></ReliableActionForm></section>
    </PortalShell>
  );
}
