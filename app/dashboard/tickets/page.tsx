...
export default async function TicketsPage({
  ...
  return (
    <PortalShell
      ...
      description="Erstelle Tickets und verfolge transparent den aktuellen Bearbeitungsstand."
    >
      <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
        <form action={createTicketAction} className="surface h-fit p-6 md:p-7">
          ...
          {query.error && (
            <p role="alert" className="mt-4 rounded-xl bg-[#ef6f6c]/10 p-3 text-sm text-[#f28d8a]">
              Bitte prüfe Kategorie und Eingaben.
            </p>
          )}
          <div className="mt-6 grid gap-5">
            ...
            <p className="sr-only" aria-live="polite">Das Ticket wird nach dem Absenden geöffnet.</p>
          </div>
        </form>
        <section className="surface overflow-hidden">
          <div className="border-b border-white/[0.07] p-6"><h2 className="text-xl font-semibold">Deine Tickets</h2></div>
          {tickets.length ? (
