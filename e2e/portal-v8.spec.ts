...
test("Portal filtert Navigation, öffnet Command Palette und zeigt Übergaben", async ({ page }) => {
  ...
  await page.goto("/staff/sanktionen");
  await expect(page.getByRole("heading", { name: "Moderation bleibt eine Melonly-Aufgabe." })).toBeVisible();
  await expect(page.getByRole("link", { name: /In Melonly öffnen/ })).toBeVisible();
});
