...
export function MediaUploader({ inputName = "mediaIds", captionInputName = "mediaCaptions", initialAssets = [], single = false, label, imagesOnly = false, internal = false }: { inputName?: string; captionInputName?: string; initialAssets?: Asset[]; single?: boolean; label?: string; imagesOnly?: boolean; internal?: boolean }) {
  ...
  const runUpload = async (task: UploadTask) => {
    ...
    try {
      const signResponse = await fetch("/api/media/sign", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: file.name, mimeType: file.type, bytes: file.size, scope: internal ? "internal" : "public" }) });
  ...
  return <div className="grid gap-3">
    ...
    <label className="button button-secondary w-fit">{label || (single ? "Bild hochladen" : "Medien hinzufügen")}<input className="sr-only" type="file" multiple={!single} accept={imagesOnly ? "image/jpeg,image/png,image/webp,image/gif" : `image/jpeg,image/png,image/webp,image/gif,audio/mpeg,audio/wav,audio/ogg,audio/mp4,video/mp4,video/webm,video/quicktime${internal ? ",application/pdf" : ""}`} onChange={(event) => { upload(event.target.files); event.currentTarget.value = ""; }} /></label>
