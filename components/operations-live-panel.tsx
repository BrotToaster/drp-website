...
const LiveOperationsMap = dynamic(() => import("@/components/live-operations-map").then((module) => module.LiveOperationsMap), {
  ssr: false,
  loading: () => <div className="live-map-module-loading"><span className="submit-spinner" /> Live-Karte wird vorbereitet …</div>,
});
...
export function OperationsLivePanel({ players, events }: { players: OperationsPlayer[]; events: OperationsEvent[] }) {
  const [tab, setTab] = useState<"players" | "events" | "map">("players");
  ...
  return <section className="surface overflow-hidden">
    <div className="flex flex-wrap items-end gap-3 border-b border-white/[0.07] p-5">
      ...
      {tab !== "map" && <label className="min-w-[210px] text-[10px] font-bold uppercase tracking-[.1em] text-[#676d72]">Suchen<input className="field mt-1 !min-h-10" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Spieler, Callsign, Ereignis" /></label>}
